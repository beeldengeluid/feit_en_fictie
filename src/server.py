...
tess_api = TessApi(
    endpoint = app.config["TESS_API"],
    api_key = app.config["TESS_KEY"]
)
...
def extract_terms():
    ...
    try:
        if extractor == "spinque":
            ...
            data = tess_api.extract_terms(
                title = title,
                text = text,
                genre = app.config["TESS_GENRE"]
            )
        else:
            raise ValueException("Invalid term extractor")
    except ApiException:
...
"""
Example:
http://localhost:5000/api/media_for_article?url=https://www.nu.nl/midden-oostenconflict/5264750/25-doden-en-minstens-duizend-gewonden-bij-palestijnse-protesten-gazastrook.html
"""
